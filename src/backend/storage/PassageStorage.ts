...
import { SentenceStorage } from './SentenceStorage';
...
export class PassageService {
  ...
  private sentenceStorage: SentenceStorage;

  constructor(prisma?: PrismaClient | Omit<PrismaClient, ITXClientDenyList>) {
    this.prisma = prisma || new PrismaClient();
    this.sentenceStorage = new SentenceStorage(this.prisma);
  }
  ...
  async savePassage(passageData: {
    ...
    try {
      ...
      for (const sentence of sentences) {
        await this.sentenceStorage.saveSentence(sentence, existingPassage.id);
      }
