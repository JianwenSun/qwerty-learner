...
export function useDictionaryStats(dictID: string, isStartLoad: boolean) {
  const [sentenceDictionaryStats, setSentenceDictionaryStats] = useState<ISentenceDictionaryStats | null>(null)

  useEffect(() => {
    const fetchDictStats = async () => {
      const stats = await getDictStats(dictID)
      setSentenceDictionaryStats(stats)
    }

    if (isStartLoad && !sentenceDictionaryStats) {
      fetchDictStats()
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dictID, isStartLoad])

  return sentenceDictionaryStats
}

interface ISentenceDictionaryStats {
  exercisedChapterCount: number
}

async function getDictStats(dict: string): Promise<ISentenceDictionaryStats> {
  try {
