...
export default function Translation({ translation, showTrans = true, onMouseEnter, onMouseLeave }: TranslationProps) {
  ...
  const { speak, speaking } = useSpeech(translation, speechOptions)
  ...
  return (
    <div className={`flex items-center justify-center  pb-4 pt-5`} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      <span
        className={`max-w-4xl text-center font-sans transition-colors duration-300 ${
          isOpenDarkMode ? 'text-white text-opacity-80' : 'text-gray-600'
